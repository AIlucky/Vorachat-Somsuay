---
description: >-
  setuid can allow a user to execute a program or script with the permissions of
  another user, typically with elevated privileges.
---
...
* We might be able to reverse engineer the program with SETUID bit set to escalate our privilege
* setgid is another special permission that allows us to run as a part of group that created them
  * ```shell-session
    find / -uid 0 -perm -6000 -type f 2>/dev/null
    ```

leveraged in the same manner as setuid to escalage privilege
...
* list of binaries and scripts that can be used by an attacker to bypass security restrictions.
* Gives us info on how to break out of restricted shell, escalate privilege, spawn reverse shell and transfer files

e.g. apt-get can be used to break out of restriced environments and spawn a shell by adding a Pre-Invoke command
